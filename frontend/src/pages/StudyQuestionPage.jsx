      ...
      <div className="mesh-bg-blue" style={{ minHeight: '100vh', background: 'white' }}>
        <div className="container" style={{ paddingTop: '2rem', paddingBottom: '3rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '3rem' }}>
            ...
            <h1 style={{ fontSize: '2rem', fontWeight: 900 }}>Review Arena</h1>
          </div>

          <div className="bento-card" style={{ padding: '2rem', marginBottom: '3rem' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(60px, 1fr))', gap: '1rem' }}>
              {questions.map((q, i) => (
                <button
                  key={i}
                  onClick={() => { setShowReview(false); setCurrentIdx(i); }}
                  style={{
                    height: '60px',
                    borderRadius: '1rem',
                    border: '1px solid hsl(var(--card-border))',
                    background: answers[q.id] !== undefined ? 'hsl(var(--primary))' : 'white',
                    color: answers[q.id] !== undefined ? 'white' : 'hsl(var(--foreground))',
                    fontWeight: 900,
                    cursor: 'pointer'
                  }}
                >
          ...
          <button className="btn-elite btn-elite-primary" style={{ width: '100%', height: '72px', fontSize: '1.2rem' }} onClick={handleSubmit}>
            Confirm & Submit Arena <Send size={24} style={{ marginLeft: '1rem' }} />
          </button>
        </div>
      </div>
    );
    ...
    <div className="mesh-bg-blue" style={{ minHeight: '100vh', background: 'white' }}>
      <div className="container" style={{ paddingTop: '2rem', paddingBottom: '3rem', display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3rem' }}>
          ...
          <button className="flex-center" style={{ width: '48px', height: '48px', background: 'rgba(239, 68, 68, 0.05)', borderRadius: '1rem', border: '1px solid rgba(239, 68, 68, 0.1)', color: '#ef4444' }} onClick={() => setShowReview(true)}>
            <Send size={20} />
          </button>
        </div>
        ...
        <div style={{ marginTop: '4rem', display: 'flex', gap: '1.5rem', maxWidth: '600px', width: '100%' }}>
          ...
          {currentIdx === questions.length - 1 ? (
            <button className="btn-elite btn-elite-primary" style={{ flex: 2, height: '64px' }} onClick={() => setShowReview(true)}>
              Review Arena <ChevronRight size={20} style={{ marginLeft: '0.75rem' }} />
            </button>
          ) : (
